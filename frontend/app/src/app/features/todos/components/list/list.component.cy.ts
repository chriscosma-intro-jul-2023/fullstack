...
describe(ListComponent.name, () => {
  describe('Utopian State', () => {
    ...
    it('The Second Item is For Now', () => {
      cy.get('li')
        .eq(1)
        .find('button')
        .should('contain.text', utopianList[1].status);
    });
    it('The Third Item is For Waiting', () => {
      cy.get('li')
        .eq(2)
        .find('button')
        .should('contain.text', utopianList[2].status);
    });
    it('The Fourth Item is For Completed', () => {
      cy.get('li')
        .eq(3)
        .find('button')
        .should('contain.text', utopianList[3].status);
    });
